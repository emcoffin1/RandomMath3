...
# --- helper function ---
def tipping_velocity(legA, legB, cg, I_cg, m):
    # axis unit vector
    ...
    theta = np.arccos(np.dot(r, h_vec) / (np.linalg.norm(r)*h))
    v = np.sqrt((2*g*dh*I_axis) / (m*d**2*np.sin(theta)**2))
...
for A, B, name in edges:
    v, I_axis, d, h, dh = tipping_velocity(A, B, cg, I_cg, m)
